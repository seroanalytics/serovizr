...
test_that("GET /dataset<name>/?public=TRUE returns 404 if dataset not found", {
  router <- build_routes(cookie_key)
  res <- router$call(make_req("GET",
                              "/dataset/testdataset/",
                              qs="public=TRUE",
...
test_that("can GET /dataset<name>/?public=TRUE", {
  ...
  body <- jsonlite::fromJSON(res$body)
  expect_equal(body$data$biomarkers, "sVNT")
